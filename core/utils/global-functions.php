 ...
 * @param string $user_permission Nome da permissão do usuário
 * @return bool True se o usuário tem a permissão necessária, False caso contrário
 */
function check_permissions($required = '', $user_permission = '')
{
	if (empty($required) || empty($user_permission)) {
		Log::info('Vazio check_permissions', 'Required: ' . $required . ' User permission: ' . $user_permission);
		return false;
	}

	if ($user_permission == 'SUPERADMIN') {
		return true;
	}

	if ($required == 'ADMIN' && $user_permission == 'ADMIN') {
		return true;
	}

	if ($required == 'USUARIO' && ($user_permission == 'USUARIO' || $user_permission == 'ADMIN')) {
		return true;
	}
	Log::info('Falha check_permissions', 'Required: ' . $required . ' User permission: ' . $user_permission);
	return false;
}
