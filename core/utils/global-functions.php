...
// Sistema simplificado de permissões para TCC
$permissions = [
	1 => 'SUPERADMIN',   // Administrador total do sistema
	2 => 'ADMIN',        // Administrador do evento
	3 => 'USUARIO'       // Usuário comum/operador
];
 ...
 * @param string $required Nome da permissão requerida ou nível mínimo
 * @param mixed $user_permission ID ou nome da permissão do usuário
 * @return bool
 */
function check_permissions($required = '', $user_permission = '')
{
	// Array global de permissões
	global $permissions;
	
	// Se não tiver permissão, retorna false
	if (empty($user_permission)) {
		return false;
	}
	
	// Pega o ID da permissão do usuário
	$user_perm_id = is_numeric($user_permission) ? 
		$user_permission : 
		array_search($user_permission, $permissions);
	
	// Pega o ID da permissão requerida
	$required_id = is_numeric($required) ? 
		$required : 
		array_search($required, $permissions);
	
	// Se não encontrou a permissão, retorna false
	if ($required_id === false || $user_perm_id === false) {
		return false;
	}
	
	// SUPERADMIN (1) tem acesso a tudo
	if ($user_perm_id == 1) {
		return true;
	}
	
	// ADMIN (2) tem acesso a tudo exceto configurações de SUPERADMIN
	if ($user_perm_id == 2 && $required_id >= 2) {
		return true;
	}
	
	// USUARIO (3) só tem acesso ao que é de USUARIO
	if ($user_perm_id == 3 && $required_id == 3) {
		return true;
	}
