...
class UsuariosModel extends MainModel
{
	...
	private $ClasseEmail;
	private $ClasseMailer;
	private $ClasseWhatsapp;
	...
	public function validarFormUsuarios()
	{
		...
		$idHash = chk_array($this->parametros, 1);
		$idPessoa = decryptHash($idHash);

		$this->idPessoa = !empty($idPessoa) ? (int) $idPessoa : null;
	...
	public function validarFormRecuperarSenha()
	{
		...
		$this->identifier = isset($_POST["identifier"]) ? $_POST["identifier"] : null;
		...
		if (filter_var($this->identifier, FILTER_VALIDATE_EMAIL)) {
			...
			$this->celular = $this->identifier;
		} else {
			$this->cpf = $this->identifier;
		}

		if (empty($this->identifier)) {
			$this->erro .= "<br>Preencha o e-mail ou celular.";
		}
		if (!empty($this->email) || !empty($this->celular)) {
			$query = $this->db->query('SELECT * FROM `vwLogin` WHERE `email` = ? OR `celular` = ? OR `cpf` = ?', array($this->email, $this->celular, $this->cpf));
		}

		$registro = $query->fetch();
		if (empty($registro)) {
			$this->erro .= "<br>Usuário não encontrado.";
		}
		...
		if (empty($this->email)) {
			$this->recuperarSenhaEmail();
		} else if (!empty($this->celular)) {
			$this->recuperarSenhaCelular();
		} else if (!empty($this->cpf)) {
			// Se o usuário tiver um telefone cadastrado, envia um código para o celular
			if (!empty($registro['celular'])) {
				$this->recuperarSenhaCelular();
			} else {
				// Se o usuário não tiver um telefone cadastrado, envia um código para o e-mail
				$this->recuperarSenhaEmail();
			}
		}
	...
	private function recuperarSenhaEmail()
	{
		...
		if (!empty($this->email)) {
			// Se o usuário tentou recuperar a senha com o e-mail, envia o email para o e-mail cadastrado
			$enviaEmail = $this->ClasseMailer->send($this->email, $assunto, $mensagem);
		} else {
			// Caso o usuário não tenha informado o e-mail, envia o email para os e-mails cadastrados
	...
	private function recuperarSenhaCelular()
	{
		$token = sha1(uniqid(time()));
		...
		$telefone = $this->form_data['celular'];
		$this->ClasseWhatsapp = new WhatsappSendBiz($this->db);

		if (!empty($telefone)) {
			// Se o usuário tentou recuperar a senha com o celular, envia o código para o celular cadastrado
			$enviaCelular = $this->ClasseWhatsapp->enviarLinkRecuperacaoSenha($telefone, $mensagem);
		} else {
			// Caso o usuário não tenha informado o celular, envia o código para os celulares cadastrados
			$celulares = $this->ClasseTelefone->getTelefones(chk_array($this->form_data, 'idPessoa'));

			foreach ($celulares as $dadosCelulares) {
				$enviaCelular = $this->ClasseWhatsapp->enviarLinkRecuperacaoSenha($dadosCelulares['celular'], $mensagem);
			}
		}
	}
	...
	private function trocarSenha($idPessoa = null)
	{
		if (is_numeric($idPessoa) && $idPessoa > 0) {
			...
			$registro = $getPessoaService->getPessoa($idPessoa);
