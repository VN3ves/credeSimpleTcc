...
class PermissoesModel extends MainModel
{
	...
	public function getPermissoes($filtros)
	{
		...
		$idPermissao = $filtros['idPermissao'];
		...
		WHERE `status`= ?
		AND `id` = ?
		ORDER BY `permissao`", array($status, $idPermissao));
	...
	public function bloquearPermissao()
	{
		...
		if (!empty($id)) {
			...
			$this->form_msg .= '<script type="text/javascript">window.location.href = "' . HOME_URI . '/permissoes/";</script>';
